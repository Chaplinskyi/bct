...
public class BarcodeService {
    ...
    public void updateStatus(Long id, String newStatus) {
        ...
        String oldStatus = barcode.getStatus(); // Запам'ятовуємо старий статус
        LocalDateTime now = LocalDateTime.now(); // Поточний час

        // Не робимо нічого, якщо статус не змінився
        if (oldStatus.equalsIgnoreCase(newStatus)) {
            log.warn("Status for barcode {} is already '{}'. No update performed.", barcode.getCode(), newStatus);
            return;
        }

        barcode.setStatus(newStatus);
        barcode.setLastUpdated(now); // Завжди оновлюємо дату останньої зміни

        // --- ЛОГІКА ДЛЯ discardDate ---
        if ("out".equalsIgnoreCase(newStatus)) {
            // Якщо новий статус 'out', встановлюємо дату списання
            barcode.setDiscardDate(now);
            log.info("Setting discardDate for barcode {}", barcode.getCode());
        } else if ("out".equalsIgnoreCase(oldStatus)) {
            // Якщо СТАРИЙ статус був 'out', а новий - ні, скидаємо дату списання
            barcode.setDiscardDate(null);
            log.info("Clearing discardDate for barcode {}", barcode.getCode());
        }
        // --- Кінець логіки discardDate ---

        // Зберігаємо змінений штрих-код
        Barcode savedBarcode = barcodeRepository.save(barcode);

        // Створюємо та зберігаємо запис в історію статусів
        // Важливо створювати історію ПІСЛЯ збереження основного об'єкта,
        // щоб мати коректний зв'язок і уникнути проблем транзакцій
        StatusHistory history = new StatusHistory(savedBarcode, oldStatus, newStatus, now);
        statusHistoryRepository.save(history); // Зберігаємо історію окремо

        log.info("Successfully updated status for barcode {} from '{}' to '{}'", savedBarcode.getCode(), oldStatus, newStatus);
    }
    ...
    @Transactional // Дуже важливо для операцій видалення!
    public void deleteAllBarcodesAndHistory() {
        log.warn("!!! Starting complete deletion of barcode history and barcodes !!!"); // Додайте логування

        // 1. Видаляємо історію (спочатку залежні сутності)
        // Використовуємо стандартні методи deleteAllInBatch для ефективності,
        // АБО ваші кастомні @Query методи, якщо стандартні не працюють як очікується
        locationHistoryRepository.deleteAllInBatch(); // Видаляє всі записи LocationHistory
        statusHistoryRepository.deleteAllInBatch();   // Видаляє всі записи StatusHistory

        // 2. Видаляємо самі штрих-коди
        barcodeRepository.deleteAllInBatch();       // Видаляє всі записи Barcode

        // 3. Опціонально: Видаляємо історію імпортів
        // importBatchRepository.deleteAllInBatch(); // Якщо потрібно видаляти й історію завантажень
        importBatchRepository.deleteAllInBatch(); // <-- ДОДАНО ЦЕЙ РЯДОК
        log.info("Import batches deleted."); // Додатковий лог

        log.warn("!!! Finished complete deletion !!!");
    }

    public List<ApnSummaryDto> getApnSummaryData() {
        log.info("Fetching APN summary data."); // Додаємо логування
        return barcodeRepository.getApnSummary();
    }

    public List<Barcode> getBarcodesForCheck() {
        log.info("Fetching barcodes with status 'check'."); // Додаємо логування
        return barcodeRepository.findAllByStatusCheck();
    }

    public Page<Barcode> findAllByOrderByCreationDateAsc(Pageable pageable) {
        log.info("Fetching all barcodes history page {} size {}", pageable.getPageNumber(), pageable.getPageSize());
        return barcodeRepository.findAllByOrderByCreationDateAsc(pageable);
    }

    @Transactional
    public int saveHistoryBarcodes(List<Barcode> barcodes) {
        if (barcodes == null || barcodes.isEmpty()) {
            return 0;
        }
        log.info("Saving {} barcodes directly to history.", barcodes.size());
        // Можна додати перевірку на існування коду тут, якщо потрібно уникати дублікатів
        List<Barcode> saved = barcodeRepository.saveAll(barcodes);
        return saved.size();
    }

    public List<RecentActivityDto> getRecentActivity(int limit) {
        // 1. Отримати останні зміни локацій
        Pageable locationPageable = PageRequest.of(0, limit, Sort.by("changeTime").descending());
        List<LocationHistory> recentLocations = locationHistoryRepository.findAll(locationPageable).getContent();

        // 2. Отримати останні зміни статусів
        Pageable statusPageable = PageRequest.of(0, limit, Sort.by("changeTime").descending());
        List<StatusHistory> recentStatuses = statusHistoryRepository.findAll(statusPageable).getContent();

        // 3. Перетворити їх у DTO
        Stream<RecentActivityDto> locationStream = recentLocations.stream()
                .filter(lh -> lh.getBarcode() != null) // Перевірка, що Barcode не null
                .map(lh -> new RecentActivityDto(
                        lh.getBarcode().getId(),
                        lh.getBarcode().getCode(),
                        lh.getOldLocation(),
                        lh.getNewLocation(),
                        lh.getChangeTime()
                ));

        Stream<RecentActivityDto> statusStream = recentStatuses.stream()
                .filter(sh -> sh.getBarcode() != null) // Перевірка, що Barcode не null
                .map(sh -> new RecentActivityDto(
                        sh.getBarcode().getId(),
                        sh.getBarcode().getCode(),
                        sh.getOldStatus(),
                        sh.getNewStatus(),
                        sh.getChangeTime(),
                        true // Додатковий параметр для розрізнення конструкторів
                ));

        // 4. Об'єднати, відсортувати за часом (від нових до старих) та взяти ліміт
        List<RecentActivityDto> combinedActivities = Stream.concat(locationStream, statusStream)
